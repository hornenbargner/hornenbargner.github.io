# Quantum: What You Need To Know To Understand SSH
...
Things are slightly more complicated in our classical model. Here, $\Psi(x,t)$ is \textit{not} a typical position function $x(t)$. You do not input a time and get a definite location. Instead, $\Psi(x,t)$ is a \textbf{wave function}, and the output for each $x,t$ contains information about the probability that the particle will be found at $x$ at time $t$. Notice I said "contains information about the probability" and not "is the probability." We will discuss how to extract the probabilities later. \\
## An Example: The Particle In A Box
Suppose that you have a 1-D particle, which you've put in a box. Our box is an \textbf{infinite square well}, denoted
\[
    V(x)= 
\begin{cases}
    0,& 0\leq x \leq a\\
...
So, having our solution $\Psi(x,t)$, how do we use this to actually understand our particle in a box? Well, we interpret information from $\Psi(x,t)$ by applying \textbf{operators} to it. One fundamental example is the energy operator, $\hat{H}$, which we call the \textbf{Hamiltonian}. Operators rely on a specific idea: since our solutions to Schrodinger's equation are a linear combination of functions (particularly orthogonal functions), we may think of the solutions as being in an infinite-dimensional vector space. Operators are actually linear operators on the vector space of wave functions. The \textbf{eigenvectors} of operators have \textbf{eigenvalues} that represent possible measurable values in an experiment. For the Hamiltonian, our energy operator, the eigenvalues $E_n$ such that
